...
mongoose.connect(MONGO_URI)
  .then(() => {
    ...
    console.log('Database: smartstorepay');
  })
  .catch(err => {
...
async function seedProducts() {
  try {
    ...
    if (count > 0) {
      console.log(`✓ Products already seeded (${count} products found)`);
      return;
    }

    console.log('Seeding products...');
    const demoProducts = [
      // Beverages (in cents: $5-$8)
      { name: 'Water Bottle', price: 500, category: 'Beverages', emoji: '💧', active: true },
      { name: 'Orange Juice', price: 650, category: 'Beverages', emoji: '🍊', active: true },
      { name: 'Coffee', price: 750, category: 'Beverages', emoji: '☕', active: true },
      // Food (in cents: $8-$15)
      { name: 'Sandwich', price: 850, category: 'Food', emoji: '🥪', active: true },
      { name: 'Burger', price: 1200, category: 'Food', emoji: '🍔', active: true },
      { name: 'Pizza', price: 1000, category: 'Food', emoji: '🍕', active: true },
      // Snacks (in cents: $2-$5)
      { name: 'Chocolate', price: 250, category: 'Snacks', emoji: '🍫', active: true },
      { name: 'Cookies', price: 350, category: 'Snacks', emoji: '🍪', active: true },
      { name: 'Popcorn', price: 400, category: 'Snacks', emoji: '🍿', active: true },
      // Stationery (in cents: $1-$5)
      { name: 'Notebook', price: 350, category: 'Stationery', emoji: '📓', active: true },
      { name: 'Pen Set', price: 450, category: 'Stationery', emoji: '🖊️', active: true },
      { name: 'Pencil', price: 150, category: 'Stationery', emoji: '✏️', active: true },
      // Electronics (in cents: $15-$80)
      { name: 'USB Cable', price: 1500, category: 'Electronics', emoji: '🔌', active: true },
      { name: 'Earbuds', price: 3500, category: 'Electronics', emoji: '🎧', active: true },
      { name: 'Charger', price: 2500, category: 'Electronics', emoji: '🔋', active: true },
      // Personal Care (in cents: $3-$8)
      { name: 'Hand Sanitizer', price: 400, category: 'Personal Care', emoji: '🧴', active: true },
      { name: 'Tissues', price: 250, category: 'Personal Care', emoji: '🧻', active: true },
      { name: 'Soap', price: 300, category: 'Personal Care', emoji: '🧼', active: true },
      // Clothing (in cents: $15-$50)
      { name: 'T-Shirt', price: 2500, category: 'Clothing', emoji: '👕', active: true },
      { name: 'Cap', price: 1500, category: 'Clothing', emoji: '🧢', active: true },
      { name: 'Socks', price: 800, category: 'Clothing', emoji: '🧦', active: true },
      // Sports (in cents: $10-$60)
      { name: 'Yoga Mat', price: 3500, category: 'Sports', emoji: '🧘', active: true },
      { name: 'Jump Rope', price: 1200, category: 'Sports', emoji: '🤸', active: true },
      { name: 'Tennis Ball', price: 1500, category: 'Sports', emoji: '🎾', active: true }
    ];
...
// Clear products (for testing/reset)
app.delete('/products/clear', async (req, res) => {
  try {
    await Product.deleteMany({});
    console.log('✓ Products collection cleared');
    res.json({ success: true, message: 'Products cleared' });
  } catch (err) {
...
server.listen(PORT, '0.0.0.0', async () => {
  ...
  if (mongoose.connection.readyState === 1) {
    ...
    console.error('✗ MongoDB connection timeout - products not seeded');
  }
});
